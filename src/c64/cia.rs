    ...
    pub fn read_register(&mut self, addr: u16, on_cia_read: &mut cpu::Callback) -> u8 {
        ...
        match addr & 0x00FF {
            ...
            0x0D => {
                ...
                self.icr = 0;
                *on_cia_read = if self.is_cia1 {
                    cpu::Callback::ClearCIAIrq
                } else {
                    cpu::Callback::ClearNMI
                };
                curr_icr
            }
    ...
    pub fn write_register(&mut self, addr: u16, value: u8, on_cia_write: &mut cpu::Callback) {
        match addr & 0x00FF {
            ...
            0x0C => {
                ...
                if irq_triggered {
                    *on_cia_write = if self.is_cia1 {
                        cpu::Callback::TriggerCIAIrq
                    } else {
                        cpu::Callback::TriggerNMI
                    };
                }
            }
            0x0D => {
    ...
    fn read_cia1_register(&mut self, addr: u16) -> u8 {
        match addr {
            0xDC00 => {
                ...
                if tst & 0x01 == 0 {
                    retval &= self.rev_matrix[0];
                }
                if tst & 0x02 == 0 {
                    retval &= self.rev_matrix[1];
                }
                if tst & 0x04 == 0 {
                    retval &= self.rev_matrix[2];
                }
                if tst & 0x08 == 0 {
                    retval &= self.rev_matrix[3];
                }
                if tst & 0x10 == 0 {
                    retval &= self.rev_matrix[4];
                }
                if tst & 0x20 == 0 {
                    retval &= self.rev_matrix[5];
                }
                if tst & 0x40 == 0 {
                    retval &= self.rev_matrix[6];
                }
                if tst & 0x80 == 0 {
                    retval &= self.rev_matrix[7];
                }
            ...
            0xDC01 => {
                ...
                if tst & 0x01 == 0 {
                    retval &= self.key_matrix[0];
                }
                if tst & 0x02 == 0 {
                    retval &= self.key_matrix[1];
                }
                if tst & 0x04 == 0 {
                    retval &= self.key_matrix[2];
                }
                if tst & 0x08 == 0 {
                    retval &= self.key_matrix[3];
                }
                if tst & 0x10 == 0 {
                    retval &= self.key_matrix[4];
                }
                if tst & 0x20 == 0 {
                    retval &= self.key_matrix[5];
                }
                if tst & 0x40 == 0 {
                    retval &= self.key_matrix[6];
                }
                if tst & 0x80 == 0 {
                    retval &= self.key_matrix[7];
                }
