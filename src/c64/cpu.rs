...
impl CPU {
    ...
    pub fn set_references(
        &mut self,
        memref: memory::MemShared,
        vicref: vic::VICShared,
        cia1ref: cia::CIAShared,
        cia2ref: cia::CIAShared,
        sidref: sid::SIDShared,
    ) {
        ...
        match self.state {
            ...
            CPUState::PerformRMW => {
                match self.instruction.cycles_to_rmw {
                    ...
                    _ => panic!(
                        "Too many cycles in RMW stage! ({}) ",
                        self.instruction.cycles_to_rmw
                    ),
                }
    ...
    pub fn write_byte(&mut self, addr: u16, value: u8) -> bool {
        ...
        if io_enabled {
            match addr {
                /*   VIC-II  */
                0xD000..=0xD3FF => as_mut!(self.vic_ref).write_register(addr, value, &mut on_write),
                /*    SID    */
                0xD400..=0xD7FF => as_mut!(self.sid_ref).write_register(addr, value),
                /* color RAM */
                0xD800..=0xDBFF => {
    ...
    pub fn read_byte(&mut self, addr: u16) -> u8 {
        ...
        if io_enabled {
            match addr {
                /*  VIC-II   */
                0xD000..=0xD3FF => byte = as_mut!(self.vic_ref).read_register(addr),
                /*   SID     */
                0xD400..=0xD7FF => byte = as_mut!(self.sid_ref).read_register(addr),
                /* color RAM */
                0xD800..=0xDBFF => {
                    byte = (as_ref!(self.mem_ref).read_byte(addr) & 0x0F)
                        | (as_ref!(self.vic_ref).last_byte & 0xF0)
                }
                /*   CIA1    */
                0xDC00..=0xDCFF => byte = as_mut!(self.cia1_ref).read_register(addr, &mut on_read),
                /*   CIA2    */
                0xDD00..=0xDDFF => byte = as_mut!(self.cia2_ref).read_register(addr, &mut on_read),
    ...
    fn process_irq(&mut self, is_nmi: bool) -> bool {
        ...
        let cycle_cnt = if is_nmi {
            self.nmi_cycles_left
        } else {
            self.irq_cycles_left
        };
        ...
        match cycle_cnt {
            ...
            2 => {
                if self.ba_low {
                    return false;
                } // TODO: is reading whole word ok in cycle 1?
            }
            1 => {
                if self.ba_low {
                    return false;
