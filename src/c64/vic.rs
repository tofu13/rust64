...
impl VIC {
    ...
    pub fn read_register(&self, addr: u16) -> u8 {
        match addr {
            ...
            0xD011 => {
                let curr_val = as_ref!(self.mem_ref)
    ...
    pub fn write_register(&mut self, addr: u16, value: u8, on_vic_write: &mut cpu::Callback) {
        ...
        match addr {
            ...
            0xD040..=0xD3FF => {
                self.write_register(0xD000 + (addr % 0x0040), value, on_vic_write);
            }
            _ => as_mut!(self.mem_ref)
                .get_ram_bank(memory::MemType::Io)
                .write(addr, value),
        }
    }
    ...
    pub fn read_byte(&mut self, addr: u16) -> u8 {
        ...
        if (va & 0x7000) == 0x1000 {
            ...
            self.last_byte = as_mut!(self.mem_ref)
                .get_rom_bank(memory::MemType::Chargen)
                .read(addr);
        } else {
            self.last_byte = as_mut!(self.mem_ref)
        ...
        match self.curr_cycle {
            ...
            1 => {
                if self.raster_cnt == (NUM_RASTERLINES - 1) {
                    ...
                    self.draw_this_line = (self.raster_cnt >= FIRST_DISP_LINE)
                        && (self.raster_cnt <= LAST_DISP_LINE)
                        && !self.frame_skipped;
                }
            ...
            58 => {
                ...
                for i in 0..8 {
                    ...
                    if ((self.sprite_dma_on & mask) != 0)
                        && ((self.raster_cnt & 0x00FF) == self.my[i] as u16)
                    {
                        self.sprite_display_on |= mask;
                    }
    ...
    fn dbg_check_regs(&mut self, addr: u16, value: u8) {
        self.dbg_reg_changed = as_mut!(self.mem_ref)
            .get_ram_bank(memory::MemType::Io)
            .read(addr)
            != value;
    }
    ...
    fn matrix_access(&mut self, c64_cycle_cnt: u32) {
        if as_ref!(self.cpu_ref).ba_low {
            if (c64_cycle_cnt - self.first_ba_cycle) < 3 {
                ...
                self.color_line[self.ml_idx] = as_ref!(self.mem_ref)
                    .get_ram_bank(memory::MemType::Io)
                    .read(0xD800 + (addr & 0x03FF));
            }
        }
    }
    ...
    fn graphics_access(&mut self) {
        ...
        if self.display_state {
            ...
            if (ctrl1 & 0x20) != 0 {
                ...
                // text
                addr =
                    ((self.matrix_line[self.ml_idx] as u16) << 3) | self.char_base | self.row_cnt;
            }
    ...
    fn draw_graphics(&mut self) {
        ...
        match self.display_mode {
            ...
            5 => {
                // invalid multicolor text
                utils::memset8(
                    &mut self.window_buffer,
                    self.screen_chunk_offset + self.x_scroll as usize,
                    0,
                );

                if (self.color_data & 8) != 0 {
                    self.fg_mask_buffer[self.fg_mask_offset] |=
                        ((self.gfx_data & 0xAA) | (self.gfx_data & 0xAA) >> 1) >> self.x_scroll;
                    self.fg_mask_buffer[self.fg_mask_offset + 1] |= ((self.gfx_data & 0xAA)
                        | (self.gfx_data & 0xAA) >> 1)
                        << (8 - self.x_scroll);
                } else {
                    self.fg_mask_buffer[self.fg_mask_offset] |= self.gfx_data >> self.x_scroll;
                    self.fg_mask_buffer[self.fg_mask_offset + 1] |=
                        self.gfx_data << (7 - self.x_scroll);
                }
            }
            6 => {
            ...
            7 => {
                ...
                self.fg_mask_buffer[self.fg_mask_offset] |=
                    ((self.gfx_data & 0xAA) | (self.gfx_data & 0xAA) >> 1) >> self.x_scroll;
                self.fg_mask_buffer[self.fg_mask_offset + 1] |=
                    ((self.gfx_data & 0xAA) | (self.gfx_data & 0xAA) >> 1) << (8 - self.x_scroll);
            }
    ...
    fn draw_std(&mut self, color: &[u8]) {
        ...
        self.window_buffer[screen_pos + 7] =
            utils::fetch_c64_color_rgba(color[(data & 1) as usize]);
        data >>= 1;
        self.window_buffer[screen_pos + 6] =
            utils::fetch_c64_color_rgba(color[(data & 1) as usize]);
        data >>= 1;
        self.window_buffer[screen_pos + 5] =
            utils::fetch_c64_color_rgba(color[(data & 1) as usize]);
        data >>= 1;
        self.window_buffer[screen_pos + 4] =
            utils::fetch_c64_color_rgba(color[(data & 1) as usize]);
        data >>= 1;
        self.window_buffer[screen_pos + 3] =
            utils::fetch_c64_color_rgba(color[(data & 1) as usize]);
        data >>= 1;
        self.window_buffer[screen_pos + 2] =
            utils::fetch_c64_color_rgba(color[(data & 1) as usize]);
        data >>= 1;
        self.window_buffer[screen_pos + 1] =
            utils::fetch_c64_color_rgba(color[(data & 1) as usize]);
        data >>= 1;
    ...
    fn draw_multi(&mut self, color: &[u8]) {
        ...
        self.fg_mask_buffer[self.fg_mask_offset + 1] |= ((((self.gfx_data & 0xAA)
            | (self.gfx_data & 0xAA) >> 1)
            as u16)
            << (8 - self.x_scroll)) as u8;

        let mut data = self.gfx_data;
        self.window_buffer[screen_pos + 7] =
            utils::fetch_c64_color_rgba(color[(data & 3) as usize]);
        data >>= 2;
            ...
            if ((self.sprite_draw & sbit) != 0) && (self.mx[snum] < (c64::SCREEN_WIDTH as u16) - 32)
            {
                ...
                if (mxe & sbit) != 0 {
                    ...
                    if (mmc & sbit) != 0 {
                        ...
                        let mut plane0_l: u32 =
                            (sdata_l & 0x55555555) | ((sdata_l & 0x55555555) << 1);
                        let mut plane1_l: u32 =
                            (sdata_l & 0xAAAAAAAA) | ((sdata_l & 0xAAAAAAAA) >> 1);
                        let mut plane0_r: u32 =
                            (sdata_r & 0x55555555) | ((sdata_r & 0x55555555) << 1);
                        let mut plane1_r: u32 =
                            (sdata_r & 0xAAAAAAAA) | ((sdata_r & 0xAAAAAAAA) >> 1);

                        // collision with graphics?
                        if ((fg_mask & (plane0_l | plane1_l)) != 0)
                            || ((fg_mask_r & (plane0_r | plane1_r)) != 0)
                        {
                            gfx_coll |= sbit;
                        ...
                        while i < 32 {
                            ...
                            if self.sprite_coll_buffer[(q + i) as usize] != 0 {
                                ...
                                self.window_buffer[(p + i as u32) as usize] =
                                    utils::fetch_c64_color_rgba(col);
                        ...
                        while i < 32 {
                            if (sdata_l & 0x80000000) != 0 {
                                ...
                                if self.sprite_coll_buffer[(q + i) as usize] != 0 {
                                    ...
                                    self.window_buffer[(p + i as u32) as usize] =
                                        utils::fetch_c64_color_rgba(color);
                                    self.sprite_coll_buffer[(q + i) as usize] = sbit;
                                }
                            }
    ...
    fn sample_border(&mut self) {
        if self.draw_this_line {
            if self.border_on {
                self.border_color_sample[(self.curr_cycle - 13) as usize] =
                    self.read_register(0xD020);
            }
