            ...
            match cpu.instruction.cycles_to_fetch {
                ...
                2 => {
                    ...
                    cpu.instruction.operand_addr =
                        ((addr_lo + cpu.x as u16) & 0xFF) | (cpu.instruction.index_addr << 8);
                    // page crossed?
        ...
        AddrMode::AbsoluteIndexedY(extra_cycle) => {
            match cpu.instruction.cycles_to_fetch {
                ...
                2 => {
                    ...
                    let addr_lo = cpu.instruction.operand_addr;
                    cpu.instruction.operand_addr =
                        ((addr_lo + cpu.y as u16) & 0xFF) | (cpu.instruction.index_addr << 8);
                    // page crossed?
        ...
        AddrMode::IndirectIndexedY(extra_cycle) => {
            match cpu.instruction.cycles_to_fetch {
                ...
                2 => {
                    ...
                    cpu.instruction.operand_addr =
                        ((opaddr + cpu.y as u16) & 0x0FF) | (cpu.instruction.index_addr << 8);
                    // page crossed?
...
pub fn get_instruction(opcode: u8) -> Option<(Op, u8, bool, AddrMode)> {
    Some(match opcode {
        ...
        /* BRK     */
        0x00 => (Op::BRK, 7, false, AddrMode::Implied),
        ...
        /* BPL_rel */
        0x10 => (Op::BPL, 4, false, AddrMode::Relative), // add 1 cycle if page boundary is crossed
        /* ORA_izy */
        ...
        /* BMI_rel */
        0x30 => (Op::BMI, 4, false, AddrMode::Relative), // add 1 cycle if page boundary is crossed
        /* AND_izy */
        ...
        /* BVC_rel */
        0x50 => (Op::BVC, 4, false, AddrMode::Relative), // add 1 cycle if page boundary is crossed
        /* EOR_izy */
        ...
        /* BVS_rel */
        0x70 => (Op::BVS, 4, false, AddrMode::Relative), // add 1 cycle if page boundary is crossed
        /* ADC_izy */
        ...
        /* BCC_rel */
        0x90 => (Op::BCC, 4, false, AddrMode::Relative), // add 1 cycle if page boundary is crossed
        ...
        /* BCS_rel */
        0xB0 => (Op::BCS, 4, false, AddrMode::Relative), // add 1 cycle if page boundary is crossed
        /* LDA_izy */
        ...
        /* BNE_rel */
        0xD0 => (Op::BNE, 4, false, AddrMode::Relative), // add 1 cycle if page boundary is crossed
        /* CMP_izy */
        ...
        /* BEQ_rel */
        0xF0 => (Op::BEQ, 4, false, AddrMode::Relative), // add 1 cycle if page boundary is crossed
        /* SBC_izy */
        ...
        /* HLT     */
        0x02 => (Op::HLT, 1, false, AddrMode::Implied),
