...
pub fn debug_instruction(opcode: u8, cpu: &mut cpu::CPU) {
    ...
    match cpu.instruction.addr_mode {
        ...
        opcodes::AddrMode::Absolute => {
            operand_hex = format!(
                " {:02X} {:02X} ",
                cpu.read_byte(prev_pc),
                cpu.read_byte(prev_pc + 0x01)
            );
    ...
    let mut total_cycles = cpu.instruction.cycles_to_fetch
        + cpu.instruction.cycles_to_run
        + cpu.instruction.cycles_to_rmw;
    ...
    let rmw_mark = if cpu.instruction.cycles_to_rmw > 0 {
        "+"
    } else {
        " "
    };
